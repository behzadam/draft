...
test.describe('Mark all as completed', () => {
  ...
  test('should allow me to mark all items as completed', async ({
    ...
    await checkNumberOfCompletedTodosInLocalStorage(
      page,
      3
    );
  });
  ...
  test('should allow me to clear the complete state of all items', async ({
    ...
    const toggleAll = page.getByLabel(
...
test.describe('Item', () => {
  ...
  test('should allow me to edit an item', async ({
    ...
    await checkTodosInLocalStorage(
      page,
      'buy some sausages'
    );
  });
...
test.describe('Persistence', () => {
  test('should persist its data', async ({ page }) => {
    ...
    const todoItems = page.getByTestId('todo-item');
    const firstTodoCheck = todoItems
      .nth(0)
      .getByRole('checkbox');
...
async function checkNumberOfTodosInLocalStorage(
  ...
  return await page.waitForFunction((e) => {
    return (
      JSON.parse(localStorage['react-todos']).length === e
    );
  }, expected);
}
